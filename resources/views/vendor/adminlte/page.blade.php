...
@section('body')
    <div class="wrapper">
                ...
                <ul class="navbar-nav ml-auto @if(config('adminlte.layout_topnav') || View::getSection('layout_topnav'))order-1 order-md-3 navbar-no-expand @endif">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right" style="left: inherit; right: 0px;">
                          <span class="dropdown-item dropdown-header">{{ Auth::user()->unreadNotifications->count() > 0 ? Auth::user()->unreadNotifications->count() : '' }} Thông báo</span>
                          <div class="dropdown-divider"></div>
                            @foreach (Auth::user()->unreadNotifications  as $notification)
                                @php ($new = json_decode(json_encode($notification->data)))
                                <a href="{{ route('customer.edit', $new->id) }}" class="dropdown-item">
                                    <i class="fas fa-user mr-2"></i> {{ $new->full_name }} vừa có một yêu cầu
                        ...
                        <li class="nav-item">
                            <a href="{{ route('user.personal') }}" class="nav-link"><b>Hi, {{ Auth::user()->name }} </b></a>
                        </li>
                        <li class="nav-item">
        ...
        <footer class="main-footer no-print">
            ...
            <strong>Copyright © {{date('Y')}} <a target="_blank" href="http://nozomijapan.vn">Nozomijapan.vn</a>.</strong> All rights
            reserved.
        </footer>
        {{-- @hasSection('footer')
